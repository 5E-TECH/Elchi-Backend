...
describe('FinanceGatewayController', () => {
  ...
  it('counts courier-sold branch orders as branch payable before courier cash is accepted', async () => {
    const { controller, financeClient, identityClient, branchClient, orderClient } =
      setup();

    financeClient.send.mockImplementation((pattern: { cmd: string }, payload: any) => {
    ...
    identityClient.send.mockImplementation((pattern: { cmd: string }, payload: any) => {
      if (pattern.cmd === 'identity.user.find_by_id' && payload.id === 'manager-1') {
        return of({ data: { id: 'manager-1', tariff_home: 10_000, tariff_center: 0 } });
      }
      if (pattern.cmd === 'identity.user.find_by_id' && payload.id === 'courier-1') {
        return of({ data: { id: 'courier-1', tariff_home: 10_000, tariff_center: 0 } });
      }
      return of({ data: {} });
    });
    ...
    orderClient.send.mockImplementation((pattern: { cmd: string }, payload: any) => {
